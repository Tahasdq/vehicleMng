...
const LoginNormalUserSchema = new mongoose.Schema({
  username: String,
  password: String,
...
// const LoginNormalUserSchemaModel = mongoose.model(
//   "LoginNormalUser",
//   ResigisterNormalUserSchema
// );
...
app.put("/updataGarrisonToTrue/:id", (req, res) => {
  ...
  NewOccuranceModel.findById(id).then((doc) => {
    ...
    NewGarissonModel.findOneAndUpdate(
      { Av_garison: doc.av_garison },
      updateFields,
      { new: false }
    ).then((NewGarisson) => {
      if (!NewGarisson) {
...
//     if (!isAdmin) {
//       const user = await ResigisterNormalUserSchemaModel.findOne({
//         username: username,
//       });
//       if (!user) {
...
//       }else{
//         return res.json({message: "ok", user: false})
//       }
//     } else if (isAdmin) {
//       const user = await ResigisterNormalUserSchemaModel.findOne({
//         username: username,
...
//           "secret123"
//         );
//         return res.json({ message: "ok", user: tokenofAdmin });
//       } else {
//         return res.json({ message: "ok", user: false });
//       }
//     }
